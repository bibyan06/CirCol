...
if (isset($_POST['id'])) {
    ...
    if ($userResult) {
        ...
        if (isset($_FILES['qr_fileInput']) && $_FILES['qr_fileInput']['error'] === UPLOAD_ERR_OK) {
            $gcash_qr = mysqli_real_escape_string($con, $_FILES['qr_fileInput']['name']);
            $updatePhotoQuery = "UPDATE users SET gcash_qr = '$gcash_qr' WHERE id = '$user_id'";
